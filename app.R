              ...
              fluidRow(
                column(
                  ...
                  box(
                    ...
                    title = "Evolução Anual Pop Idosa e Fraturas",
                  ...
                  box(
                    width = 12, solidHeader = TRUE,
                    title = "Evolução Anual Densiometria",
...
server <- function(input, output, session) {
  ...
  output$relatorio_completo <- renderUI({
    ...
    HTML(paste0(
      "<p style='font-size:18px;'><b>Total de fraturas:</b> ", total_quedas, "</p>",
      "<hr>",
      "</ul>"
    ))
  })
  ...
  output$grafico_pop_fraturas <- renderPlot({
    ...
    if (nrow(dados) == 0) {
      ...
      dados %>%
        ...
        geom_text(
          aes(label = scales::comma(valor, big.mark = ".", decimal.mark = ",")),
          vjust = -0.7,            # desloca um pouco para cima
          size  = 3.5,
          show.legend = FALSE
        ) +
  ...
  output$grafico_densito <- renderPlot({
    ...
    if (nrow(dados) == 0) {
      ...
      ggplot(dados, aes(x = ANO_CMPT, y = densitometria)) +
        ...
        geom_text(
          aes(label = scales::comma(densitometria, big.mark = ".", decimal.mark = ",")),
          vjust = -0.7,
          size  = 3.5,
          color = "#027373",
          show.legend = FALSE
